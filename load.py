...
def render_scene(texture_id, render_type="wireframe", camera=Camera([0, 0, 0], 0, 0, 10)):
    ...
    if render_type == "wireframe":
        ...
        save_image(width, height, "image_" + render_type + ".png")
        ...
        # 读取并保存深度信息
        save_depth_image(width, height, "image_" + render_type + ".png")
    elif render_type == "normal":
        glBegin(GL_TRIANGLES)
        for i, face in enumerate(faces):
            # glColor3ub(*normals_rgb[i])  # 设置颜色为对应面的RGB值
...
# 截取照片
def cropThePhoto(camera=Camera([0, 0, 0], 0, 0, 10)):
    render_type = "texture"
    render_scene(texture_id, render_type, camera)
...
def main():
    ...
    # 截取照片
    camera = Camera([center[0], center[1], center[2]], 0, 130, 1.8 * max_size)
    cropThePhoto(camera)
